...
TriangleMesh::TriangleMesh(const Json& json) : Shape(json) {
	...
	sumAreaArray = new float[primCount];
	...
	for (int primID = 0; primID < primCount; ++primID) {
		...
		Point3f vtx0 = meshData->vertexBuffer[vtx0Idx],
			vtx1 = meshData->vertexBuffer[vtx1Idx],
			vtx2 = meshData->vertexBuffer[vtx2Idx];

		Vector3f edge0 = vtx1 - vtx0, edge1 = vtx2 - vtx0;
		float area = 0.5f * cross(edge0, edge1).length();
		if (primID == 0)
			sumAreaArray[primID] = area;
		else
			sumAreaArray[primID] = sumAreaArray[primID - 1] + area;
	}
	sumArea = sumAreaArray[primCount - 1];
}
...
void TriangleMesh::uniformSampleOnSurface(Vector2f sample, Intersection* intersection, float* pdf) const
{
	...
	fillIntersection(.0f/*unuse*/, primID, u, v, intersection);
	*pdf = 1.0f / sumArea;
}
